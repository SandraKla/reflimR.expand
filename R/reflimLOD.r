...
#'
#' @param x Value where the derivative should be computed.
#' @param lambda The parameter of the Box-Cox transformation.
#'
...
#'
#' @param x Value where the density should be computed.
#' @param mu Expectation of the Normal distribution.
#' @param sigma Standard deviation of the Normal distribution.
#' @param lambda.inv The parameter of the inverse Box-Cox transformation.
#' @param lambda The parameter of the Box-Cox transformation.
#'
...
dens.bcinv.bc <- function(x, mu=5, sigma=1, lambda.inv=1, lambda=1){
  return(dnorm(box.cox.inv.trans(box.cox.trans(x,lambda = lambda.inv),lambda = lambda),mean = mu,sd = sigma)*d.box.cox.trans(box.cox.inv.trans(x,lambda = lambda),
                                                                                                                             lambda = lambda.inv)*d.box.cox.inv.trans(x,lambda = lambda))
}
...
#' Removes outliers from a vector of measured values with values below LOD
#'
...
#' Estimates the reference interval based on a maximum likelihood approach
#'
...
reflimLOD.MLE <- function(measured.values, lod, n.lod, lambda=0, right.quantile=0.75){
  ...
  obj.fun <- function(pars){
    pnorm.lod <- truncnorm::ptruncnorm(transformed.lod,b = normal.result$upper.truncation,mean = pars[1],sd = pars[2])

    return(-n.lod*log(pnorm.lod) - length(normal.result$selected.values)*log(1 - pnorm.lod) - sum(log(truncnorm::dtruncnorm(normal.result$selected.values,a = transformed.lod,
                                                                                                                            b = normal.result$upper.truncation,mean = pars[1],sd = pars[2]))))
  }
  ...
  return(list(lower.limit = lims[1],upper.limit = lims[2],percentile1 = lims1perc[1],percentile99 = lims1perc[2],mu.log = unname(optim.result$par[1]),
              ...
              selected.values = box.cox.inv.trans(normal.result$selected.values,lambda = lambda),minus.log.likelihood = optim.result$value,lod = lod,n.lod = n.lod,lambda = lambda))
}

#' Estimates the reference interval based on a quantile-based approach.
#'
#' Removes outliers from a vector of measured values with values below LOD and estimates the
#' reference interval based on a quantile-based approach. It is assumed that the values
#' from the healthy subjects follow a  normal distribution after the application of
#' pre-specified Box-Cox transformation. The default is lambda=0, i.e. it is assumed that
#' the values from the healthy subjects follow a lognormal distribution. It is also assumed
#' that the median of the values from the  healthy subjects is above LOD.
...
reflimLOD.Quant <- function(measured.values, lod, n.lod, lambda=0, right.quantile=0.75){
  ...
  return(list(lower.limit = lims[1],upper.limit = lims[2],percentile1 = lims1perc[1],percentile99 = lims1perc[2],mu.log = unname(mu),
              sigma.log = unname(sigma),upper.truncation = unname(box.cox.inv.trans(normal.result$upper.truncation,lambda = lambda)),
              selected.values = box.cox.inv.trans(normal.result$selected.values,lambda = lambda),lod = lod,n.lod = n.lod,lambda = lambda))
}

#' Computes bootstrap confidence intervals for the reference interval limits
#'
#' Similar to reflimLOD.MLE(), but computes bootstrap confidence intervals for the reference interval limits and 1% and 99% quantile
#'
#' @param measured.values The measured values above LOD.
#' @param lod Limit of Detection (LOD).
#' @param n.lod The number of values below LOD.
...
#' @param n.bootstrap Number of bootstrap samples for the computation of confidence interval.
#'
#' @return A list with the following elements:
#' \describe{
#'   \item{lower.limit.ci}{Confidence interval for the lower limit of the reference interval.}
#'   \item{upper.limit.ci}{Confidence interval for the upper limit of the reference interval.}
#'   \item{percentile1.ci}{Confidence interval for the 1% quantile of the values from the healthy subjects.}
#'   \item{percentile99.ci}{Confidence interval for the 1% quantile of the values from the healthy subjects.}
#'   \item{mu.log.ci}{Confidence interval for the expected value of the normal distribution of the Box-Cox transformed values from the healthy subjects.}
#'   \item{sigma.log.ci}{Confidence interval for the standard deviation of the normal distribution of the Box-Cox transformed values from the healthy subjects.}
#'   \item{upper.truncation.ci}{Confidence interval for the upper truncation.}
#' }
#' @import stats
#'
#' @export
ci.reflimLOD.MLE <- function(measured.values, lod, n.lod, lambda=0, right.quantile=0.75, conf.level=0.95, n.bootstrap=1000){
  ...
  return(list(lower.limit.ci = lower.limit.ci,upper.limit.ci = upper.limit.ci,percentile1.ci = percentile1.ci,percentile99.ci = percentile99.ci,
              mu.log.ci = mu.log.ci,sigma.log.ci = sigma.log.ci,upper.truncation.ci = upper.truncation.ci))
}

#' Computes the coefficient of determination for the regression line for the QQ-plot
#'
#' @param res.lodiboxplot Result object returned by the function reflimLOD.MLE().
#'
#' @return A list with the following elements:
#' \describe{
#'   \item{r.squared}{Coefficient of determination for the regression line for the QQ-plot.}
...
#' Generates a QQ-plot
#'
#' @param res.lodiboxplot Result object returned by the function reflimLOD.MLE() distribution of the values from the healthy subjects.
#' @param pch The usual graphics parameters.
#' @param line.col The usual graphics parameters.
#' @param lwd The usual graphics parameters.
...
#' @param lod Limit of Detection (LOD).
#' @param n.lod The number of values below LOD.
#' @param lambdas The lambda values for which R-squared should be computed.
#' @param adjusted Indicates whether the adjusted R-squared values should be used.
#' @param right.quantile As in reflimLOD.MLE().
#' @param lwd The usual graphics parameters.
...
#' @param ylab The usual graphics parameters.
#' @param pch The usual graphics parameters.
#' @param col The usual graphics parameters.
#' @param col.grid The usual graphics parameters.
#'
...
#' @param xlab The usual graphics parameters.
#' @param ylab The usual graphics parameters.
#' @param main The usual graphics parameters.
#' @param lwd The usual graphics parameters.
#' @param col.curve The usual graphics parameters.
...
lod.hist <- function(res.lodiboxplot, xlab="", ylab="Frequency", main="", lwd=2, col.curve=2, breaks=10, col.grid=NULL){
  ...
  if (length(mybreaks) == 1) {
    ...
    mybreaks <- seq(from = min(c(res.lodiboxplot$lod,min(breaks))),to = max(c(tmv,max(breaks))),length.out = length(breaks)) #Correction length.breaks -> length(breaks)
  }
...
#'
#' @param x The sample from the truncated normal distribution.
...
#'
#' @param n Sample size
#' @param prop.lod.healthy (Expected) Proportion of values below LOD
#' @param prop.path Proportion of values from the pathological population
