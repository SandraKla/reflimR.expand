#' Reference intervals based on a weighted sliding window technique
#'
...
#'
#' @param x Input measurement values.
#' @param t Time points or covariate values.
#' @param distribution Weight function type: "truncated_gaussian", "gaussian", "triangular", or "trapezoidal".
#' @param log.scale If TRUE, applies logarithmic scaling to the x-axis.
#' @param standard_deviation Standard deviation for Gaussian distributions (default: 5).
#' @param standard_deviation_compare Standard deviation for comparison plot.
#' @param vertex1 Shape parameters for triangular/trapezoidal distributions (0-1).
#' @param vertex2 Shape parameters for triangular/trapezoidal distributions (0-1).
#' @param vertex1_com Shape parameters for comparison plot.
#' @param vertex2_com Shape parameters for comparison plot.
#' @param window.size Window size for sliding window.
#' @param step.width Step width for sliding window.
#' @param window.size_com Window parameters for comparison plot.
#' @param step.width_com Window parameters for comparison plot.
#' @param perc.trunc Numeric. Percentage of values that were removed from each side by truncation. Default is \code{2.5}.
#' @param n.min.window Integer. Minimum number of values required in a window for a robust estimate.
#' @param n.min Integer. Minimum number of values required in \code{x.trunc} for a robust estimate. Default is \code{200}.
#' @param apply.rounding Logical. If \code{TRUE}, the reference limits are rounded. Default is \code{TRUE}.
#' @param lognormal If TRUE, assumes log-normal distribution.
#' @param weight_threshold Minimum weight sum threshold.
#' @param verbose If TRUE, prints additional information during execution.
#' @param MLE Maximum Likelihood Estimation.
#'
...
#'
#' Removes outliers from a vector of measured values with values below LOD and with weights and estimates the
...
#' the values from the healthy subjects follow a lognormal distribution. It is also assumed
#' that the median of the values from the  healthy subjects is above LOD.
#'
#' @param measured.values The measured values above LOD.
#' @param lod Limit of Detection (LOD).
#' @param n.lod The number of values below LOD.
#' @param weights Weights
#' @param lambda The lambda value for the Box-Cox transformation leading to a normal distribution of the values from the healthy subjects.
#' @param right.quantile The (iterated) estimation of the spread is based on the median and the quantile specified by right.quantile. The default is the third quartile.
#' @param verbose If TRUE, prints additional information during execution.
#'
#' @return A list with the following elements:
#' \describe{
#'   \item{lower.limit}{The estimate for the lower limit of the reference interval. If the
#'   assumption that the median of the values from the healthy subjects is
#'   above LOD is not fulfilled, NA is returned.}
#'   \item{upper.limit}{The estimate for the upper limit of the reference interval. If the
#'   assumption that the median of the values from the healthy subjects is
#'   above LOD is not fulfilled, NA is returned.}
#'   \item{percentile1}{The estimate for the 1\% quantile of the values from the healthy subjects.
#'   If the assumption that the median of the values from the healthy
#'   subjects is above LOD is not fulfilled, NA is returned.}
#'   \item{percentile99}{The estimate for the 99\% quantile of the values from the healthy
...
#'   \item{mu.log}{The estimate for the expected value of the normal distribution of the
...
#'   \item{sigma.log}{The estimate for the standard deviation of the normal distribution of
#'   the Box-Cox transformed values from the healthy subjects. If the
#'   assumption that the median of the values from the healthy subjects is
#'   above LOD is not fulfilled, NA is returned.}
#'   \item{upper.truncation}{Estimate for the 97.5\%-quantile of the healthy population. If the
#'   assumption that the median of the values from the healthy subjects is
#'   above LOD is not fulfilled, NA is returned.}
#'   \item{selected.values}{The subset of values from measured.values that are assumed to be below
...
#'   \item{lod}{LOD as it was specified in the argument of this function.}
#'   \item{n.lod}{The number of values below LOD as it was specified in the argument of
#'   this function.}
...
#'   median of the values from the healthy subjects is above LOD is not
#'   fulfilled, NA is returned.}
#'   \item{selected.values.index}{The index of the subset of values from measured.values that are assumed to be below
#'   the 97.5%-quantile of the healthy population.}
#'   \item{upper.truncation}{Estimate for the 97.5%-quantile of the healthy population. If the
#'   assumption that the median of the values from the healthy subjects is
#'   above LOD is not fulfilled, NA is returned.}
#'   \item{median}{Estimate for the median of the healthy population.}
#'   \item{lod}{LOD as it was specified in the argument of this function.}
...
#'
#' Creates a `ggplot2` visualization of sliding reference limits along a continuous time axis,
#' optionally including confidence intervals. This function is useful for inspecting the dynamics
#' of lower and upper reference limits over time, especially in the context of individualized or age-dependent reference intervals.
#'
#' @param result.sliding.reflim A list or data frame containing the sliding reference limits and their corresponding confidence intervals.
#'        Must include timepoints and values for lower and upper limits, optionally with confidence bounds.
#' @param log.scale Logical. If `TRUE`, the time axis will be plotted on a logarithmic scale.
#' @param use.mean Logical. If `TRUE`, plots the mean reference limits. Otherwise, raw estimates are used.
#' @param xlim Optional numeric vector of length 2 specifying the x-axis limits.
...
#' @param col.low RGB vector for the color of the lower reference limit line (default is blue).
#' @param col.upp RGB vector for the color of the upper reference limit line (default is red).
#' @param lwd Line width for the plotted lines.
#' @param transparency Alpha transparency level for confidence interval shading (default is 0.2).
#' @param draw.cis Logical. If `TRUE`, confidence intervals will be plotted.
#' @param grid.col Optional color for the background grid lines.
#' @param log Optional string passed to the plot to control axis scaling. Usually "" or "x".
#' @param cut.at Numeric value indicating the threshold at which to cut extreme values (used for display control).
#'
#' @import ggplot2
...
#' Plot Sliding Reference Limits with Confidence Intervals
#'
#' Used to compare results for two different parameters. Creates a `ggplot2` visualization of sliding reference limits along a continuous time axis,
#' optionally including confidence intervals. This function is useful for inspecting the dynamics
#' of lower and upper reference limits over time, especially in the context of individualized or age-dependent reference intervals.
#'
#' @param result.sliding.reflim1 A data frame or list containing the sliding reference limits and their associated confidence intervals
#' @param result.sliding.reflim2 Another data frame or list containing the sliding reference limits and their associated confidence intervals
#' @param log.scale Logical. If `TRUE`, the time axis will be plotted on a logarithmic scale.
#' @param use.mean Logical. If `TRUE`, plots the mean reference limits. Otherwise, raw estimates are used.
#' @param xlim Optional numeric vector of length 2 specifying the x-axis limits.
#' @param ylim Optional numeric vector of length 2 specifying the y-axis limits.
#' @param xlab Optional character string for the x-axis label.
#' @param ylab Optional character string for the y-axis label.
#' @param col.low1 RGB vector for the color of the lower reference limit line.
#' @param col.upp1 RGB vector for the color of the upper reference limit line
#' @param col.low2 RGB vector for the color of the lower reference limit line.
#' @param col.upp2 RGB vector for the color of the upper reference limit line.
#' @param lwd Line width for the plotted lines.
#' @param transparency Alpha transparency level for confidence interval shading (default is 0.2).
#' @param draw.cis Logical. If `TRUE`, confidence intervals will be plotted.
#' @param grid.col Optional color for the background grid lines.
...
#' @param a Left endpoint of a triangle.
#' @param b Vertex of a triangle.
#' @param c Right endpoint of the triangle.
#'
...
#'
#' @param x Numeric vector.
#' @param a Left endpoint of a trapezoid.
#' @param b Trapezoid left horizontal point.
#' @param c Trapezoid right horizontal point.
#' @param d Trapezoid right endpoint.
#'
...
#'
#' @param x Input measurement values.
#' @param covariate Time points or covariate values.
#' @param distribution Weight function type: "truncated_gaussian", "gaussian", "triangular", or "trapezoidal".
#' @param standard_deviation Standard deviation for Gaussian distributions (default: 5).
...
#'
#' @description
#' Performs a sliding window analysis using various weight functions to estimate reference limits over time or across a covariate.
#' The function supports multiple distribution types and allows comparison between different parameter configurations.
#' It is useful for analyzing trends in laboratory or clinical data while accounting for varying data density and noise.
#'
#' @param x Numeric vector of measurement values.
#' @param covariate Numeric vector representing time points or any covariate along which the analysis is performed.
#' @param distribution Character string specifying the type of weight function to apply.
#'   Options are \code{"truncated_gaussian"}, \code{"gaussian"}, \code{"triangular"}, or \code{"trapezoidal"}.
#' @param standard_deviation Numeric value specifying the standard deviation for Gaussian weight functions (default is 5).
#' @param vertex1 Numeric values between 0 and 1 that define the shape of triangular or trapezoidal weight functions.
#' @param vertex2 Numeric values between 0 and 1 that define the shape of triangular or trapezoidal weight functions.
#' @param window.size Numeric value defining the size of the sliding window.
#' @param step.width Numeric value defining the step width of the sliding window.
#' @param lognormal Logical. If \code{TRUE}, a log-normal distribution is assumed for the reference limit estimation.
#' @param perc.trunc Numeric value specifying the percentage of presumably normal values to remove from each side during truncation (default is 2.5).
#' @param n.min.window Integer specifying the minimum number of observations required in each window (default is 200).
#' @param n.min Integer specifying the overall minimum number of values required to perform the estimation (default is 100).
#' @param apply.rounding Logical. If \code{TRUE}, reference limits are rounded.
#' @param weight_threshold Numeric value specifying the minimum total weight required for a window to be considered.
#' @param verbose Logical. If \code{TRUE}, the function prints status messages during execution.
#'
...
#' Reference limits
#'
#' Estimation of reference limits from mixed distributions of normal and pathological laboratory results. Updated for weighted data.
#' Estimates lower and upper reference limits and provides statistical characteristics and graphics to evaluate the results.
#'
#' @param x Vector of positive numbers.
#' @param x_weight Weight for the vector of positive numbers.
#' @param lognormal Logical. Indicates whether a lognormal distribution should be assumed. If \code{NULL}, the distribution type is determined automatically.
#' @param targets Optional numeric vector of length 2 indicating target reference limits from external sources.
#' @param perc.trunc Numeric. Percentage of presumably normal values to be removed from each side. Default is 2.5.
#' @param n.min Integer. Minimum number of observations needed for a robust estimate of reference intervals. Default is 200.
#' @param apply.rounding Logical. If \code{TRUE}, the estimated reference limits will be rounded. Default is \code{TRUE}.
#' @param plot.it Logical. If \code{TRUE}, a graphical comparison of observed and theoretical distributions is plotted. Default is \code{TRUE}.
#' @param plot.all Logical. If \code{TRUE}, graphics of all processing steps are generated. Default is \code{FALSE}.
#' @param print.n Logical. If \code{TRUE}, the number of cases after truncation is printed on the plot. Default is \code{TRUE}.
#' @param main Character. Title of the graphic. Default is \code{"reference limits"}.
...
#'   \item{lognormal}{Logical indicating whether a lognormal distribution was assumed.}
#'   \item{limits}{Estimated reference limits with tolerance intervals.}
#'   \item{targets}{Target values with tolerance intervals (if specified).}
#'   \item{perc.norm}{Estimated percentage of non-pathological values.}
#'   \item{confidence.int}{95% confidence intervals for the estimated reference limits.}
#'   \item{interpretation}{Short text describing the deviation of observed limits from target values.}
#'   \item{remarks}{Short text describing potential reasons why the function could not be executed.}
#' }
#'
...
#' @param x_weight Weights of the numeric vector of positive numbers.
#' @param lognormal Logical. Indicates whether a lognormal distribution should be assumed. If \code{NULL}, the distribution type is determined automatically.
#' @param perc.trunc Numeric. Percentage of presumably normal values to be removed from each side. If increased (e.g. from 2.5 to 3.5), more values are excluded. Default is \code{2.5}.
#' @param apply.rounding Logical. If \code{TRUE}, estimated reference limits are rounded. Default is \code{TRUE}.
#' @param plot.it Logical. If \code{TRUE}, a graphic is created showing the iterative truncation process. Default is \code{TRUE}.
#' @param main Character. Title of the graphic. Default is \code{"iBoxplot"}.
#' @param xlab Character. Label for the x-axis. Default is \code{"x"}.
#'
#' @return A list with the following elements:
#' \describe{
#'   \item{trunc}{Truncated vector \code{x} (after removing pathological values).}
#'   \item{truncation.points}{Estimated lower and upper truncation limits (preliminary reference limits).}
#'   \item{lognormal}{Logical. Indicates whether a lognormal distribution was assumed.}
#'   \item{perc.norm}{Estimated proportion of non-pathological values.}
#'   \item{progress}{Details of the iterative truncation process.}
#' }
...
#'
#' Generates and plots a quantile-quantile plot (Q-Q plot) with a theoretical normal distribution on the x-axis
#' and the corresponding empirical distribution on the y-axis. Returns the intercept, slope, and estimated
#' 2.5% and 97.5% quantiles (i.e., preliminary reference limits).
#'
#' @param x.trunc Truncated numeric vector of positive values, usually generated by \code{iboxplot}.
#' @param x_weight Weights of the numeric vector of positive numbers.
#' @param lognormal Logical. Indicates whether a lognormal distribution should be assumed. If \code{NULL}, the type is determined automatically.
#' @param perc.trunc Numeric. Percentage of values that were removed from each side by truncation. Default is \code{2.5}.
#' @param n.min Integer. Minimum number of values required in \code{x.trunc} for a robust estimate. Default is \code{200}.
#' @param apply.rounding Logical. If \code{TRUE}, the reference limits are rounded. Default is \code{TRUE}.
#' @param plot.it Logical. If \code{TRUE}, a Q-Q plot is drawn. Default is \code{TRUE}.
#' @param main Character. Title of the plot. Default is \code{"Q-Q plot"}.
#' @param xlab Character. Label for the x-axis. Default is \code{"theoretical quantiles"}.
#' @param ylab Character. Label for the y-axis. Default is \code{"sample quantiles"}.
#'
#' @return A list with the following components:
#' \describe{
#'   \item{result}{A numeric vector containing the intercept and slope of the fitted Q-Q line, and the estimated lower and upper truncation points (reference limits).}
#'   \item{lognormal}{Logical. Indicates whether a lognormal distribution was assumed.}
#' }
#'
#' @importFrom Hmisc wtd.quantile
